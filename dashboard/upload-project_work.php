...
$links2 = $target_file2;
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    $price = mysqli_real_escape_string($connect, $_POST['price']);
            ...
            $status="OK";
            if ($status=="OK") {
            $insertme = mysqli_query($connect, "INSERT INTO project_work(school_name, faculty_name, department_name, course_title, course_code, cover_pics, course_material,Program,price,Preview) VALUES ('$school_name','$faculty_name','$department_name','$course_title','$course_code','$links', '$links1','$Program','$price','$links2')");
            if ($insertme) {
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["Preview"]["tmp_name"], $target_file2)) {
        ...
        print"<script> alert('project_work Uploaded'); </script>";
        header('Location: project_work.php');
    } else {
        echo "Sorry, there was an error uploading your file.";
    }
}
