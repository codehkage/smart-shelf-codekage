...
$(".package_post").click(function(){
          ...
          $(".CMd").click(function(){
           ...
           $("#download").click(function(){
             ...
             // $('.ppos').html("pdf = " + localStorage.getItem("pdf"));
             $.ajax({
              url : pdf,
              ...
              processData: false
               ...
               if(response.status && response.status === 200){
                savePdf(response.responseText, pdf);
               }
             });
             function savePdf(book, key) {
              var blob = new Blob([book], {type:"application/pdf"});
              var fileReader = new FileReader();
              fileReader.onload = function(evt) {
                var result = evt.target.result;
                try{
                  localStorage.setItem(key, result);
                   $('.ppos').html("pdf = " + localStorage.getItem(key));
                }
                catch(e){
