...
session_start();
   // echo "<h4> Packages</h4>";
                            ...
                            $query = ("SELECT * FROM health WHERE school = '".$school."'");
                            $result = mysqli_query($connect, $query);
                            	while ($row = mysqli_fetch_array($result)) {
                                echo'
                                <div data-toggle="modal" data-target="#myModal" class="col-sm-6 col-xs-12 books-wrapper CMd healthdetail" id="" style="margin:20px auto;width:50%;" ids="'.$row['id'].'">
            											<div class="thumbnail">
