...
session_start();
   // echo "<h4> Packages</h4>";
                            ...
                            $query = ("SELECT * FROM technology WHERE school = '".$school."'");
                            $result = mysqli_query($connect, $query);
                            	while ($row = mysqli_fetch_array($result)) {
                                echo'
                                <div data-toggle="modal" data-target="#myModal5" class="col-sm-6 col-xs-12 books-wrapper CMd technologys" style="margin:20px auto;width:50%;" id="'.$row['id'].'"">
            											<div class="thumbnail">
