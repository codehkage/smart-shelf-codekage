...
// 					mysqli_query($connect, $insertUser) or die(mysqli_error());
// 				}		
...
// 					mysqli_query($connect, $insertUser2);
// 					$data['check'] = 'true';
// 				}			
// 		$finish = time() + 7;
// 		$getRoomUsers = mysqli_query($connect,"SELECT * FROM `chat_users_rooms` WHERE `room` = '$room'");
// 		$check = mysqli_num_rows($getRoomUsers);
        	
// 	    while(true)
// 		{
// 			usleep(10000);
// 			mysqli_query($connect,"UPDATE `chat_users` SET `time_mod` = '$now' WHERE `username` = '$username'");
// 			$olduser = time() - 5;
// 			$eraseuser = time() - 30;
// 			mysqli_query($connect,"DELETE FROM `chat_users_rooms` WHERE `mod_time` <  '$olduser'");
// 			mysqli_query($connect,"DELETE FROM `chat_users` WHERE `time_mod` <  '$eraseuser'");
// 			$check = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM `chat_users_rooms` WHERE `room` = '$room' "));
// 			$now = time();
// 			if($now <= $finish)
// 			{
// 				mysqli_query($connect,"UPDATE `chat_users_rooms` SET `mod_time` = '$now' WHERE `username` = '$username' AND `room` ='$room'  LIMIT 1") ;
// 				if($check != $current){
// 				 break;
// 				}
// 			}
